...
const { generateAccessToken, generateRefreshToken } = require('../../src/middlewares/jwt');
const sendMail = require('../utils/sendMail');
const jwt = require('jsonwebtoken');
const crypto = require('crypto');
const makeToken = require('uniqid');
const { any } = require('../config/cloudinary.config');
...
//       }
//     );
//     const html = `please click to link behind to complete the register process. Link has expried after 15 minute for now.
//     <a href=${process.env.URL_SERVER}/api/user/finalregister/${token}>Click here</a>`;
//     await sendMail({ email, html, subject: "Finish Register!!" });
//     return res.json({
//       success: true,
...
const register = asyncHandler(async (req, res) => {
  ...
  if (user) throw new Error('User has existed');
    ...
    const token = makeToken();
    const emailRegex = btoa(email) + '@' + token;
...
const finalRegister = asyncHandler(async (req, res) => {
  ...
  return res.json({
    ...
    message: notActiveEmail ? 'Register is successfully , Please login' : 'Something went wrong',
  });
});
...
const login = asyncHandler(async (req, res) => {
  ...
  if (response && (await response.isCorrectPassword(password))) {
    //tach pass role ra res
    const { password, role, refreshToken, ...userData } = response.toObject();
    //tao accesstoken
    const accessToken = generateAccessToken(response._id, role);
    //tao refreshtoken
    const newrefreshToken = generateRefreshToken(response._id);
    //luu refeshtok vao cookie
...
const logout = asyncHandler(async (req, res) => {
  ...
  //xoa refresh token cookie browser
...
const forgotPassword = asyncHandler(async (req, res) => {
  ...
  const html = `please click to link behind to change password. Link has expried after 15 minute for now. 
  ...
  return res.status(200).json({
    ...
    message: rs.response?.includes('OK') ? 'Please check your email !!!' : 'Error, please try again',
  });
});
...
const resetPassword = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json({
    success: user ? true : false,
    message: user ? 'update password ' : 'somthing went wrong',
  });
});
...
const getusers = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json({
    success: response ? true : false,
    counts,
    users: response ? response : 'Không thể lấy sản phẩm',
  });
});
...
const updateCart = asyncHandler(async (req, res) => {
  ...
  if (alreadyProduct) {
    ...
    return res.status(200).json({
      success: response ? true : false,
      message: response ? 'Added to cart' : 'Something went wrong',
    });
  } else {
    const response = await User.findByIdAndUpdate(
      _id,
...
const updateWishlist = asyncHandler(async (req, res) => {
  ...
  if (alreadyWishlist) {
    ...
    return res.json({ success: response ? true : false, message: response ? 'Update Your wish list' : 'Fail to update wishlist' });
  } else {
